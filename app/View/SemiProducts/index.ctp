<?php $this->start('main_placeholder'); ?>
  ...
  <?php if ( count($semiProducts) === 0 ): ?>
    <div class="notice warning">
      <p>Trenutno nema poluproizvoda.</p>
    </div>
  <?php else: ?>
    <table class="striped">
      <thead>
      ...
      <tbody>
        <?php foreach ( $semiProducts as $semiProduct ): ?>
          <tr>
            <td><?= $semiProduct['Item']['code']; ?></td>
            <td><?= $semiProduct['Item']['name']; ?></td>
            <td><?= $semiProduct['Item']['status']; ?></td> 
            <td><?= $semiProduct['MeasurementUnit']['name']; ?></td>
            <td><?= $this->Form->postLink('Obrisi',[
              'action' => 'delete',  $semiProduct['SemiProduct']['id']
            ],[
              'class' => 'large red',
              'confirm' => 'Da li ste sigurni?'
            ]) ?></td>
            <td>
              <?= $this->Html->link('Edituj',['action' => 'save',$semiProduct['SemiProduct']['id']],[
                'class' => 'large red'
              ]) ?>
            </td>
          </tr>
